...
class Maze:
  def __init__(self, size, displaySize, loops, heuristic):
    self._size = size
    self._h = heuristic
    ...
    # Setup the canvas
    self._scale = displaySize / float(size)
    self._canvas = cs1graphics.Canvas(displaySize,displaySize)
    self._canvas.setTitle(['Zero (BFS)','Straight line','Manhattan','Shortest path in unexplored region'][int(self._h)])
  ...
  def run(self):
    ...
    while command != 'q':
      ...
      for i in range(self._size*self._size):
        ...
        if (x,y) in self._text.keys() and (x,y) not in self._explored:
          ...
          options.append((l+h,-l,'\t%d\t%d\t%6.2f\t%6.2f' % (i,l,h,l+h), i))
      ...
      command = str(options[0][3])
      branch = count**(1./options[0][0])
      #sleep(2)
    ...
    print 'Heuristic:', ['Zero (DFS)','Straight line','Manhattan','Shortest path in unexplored region'][int(self._h)]
    print 'Nodes expanded:', count
    print 'Solution length:', options[0][0]
    print 'Approximate branching factor:', branch
  ...
  def reveal(self, location):
    ...
    for (x,y) in adjacent:
      if (x,y) not in self._explored:
        ...
        t = cs1graphics.Text(label, self._scale*.3, cs1graphics.Point((x+.5)*self._scale, (y+.5)*self._scale))
...
parser.add_argument('--heuristic', help='Heurstic to use (0-3)')
parser.add_argument('--random', '-r', help='Random seed')
args = parser.parse_args()

random.seed(int(args.random))
m = Maze(args.size, args.window_size, args.loops, args.heuristic)
m.run()
