...
class MinMaxPossible4HeuristicD4 {
public:
...
int MinMaxPossible4HeuristicD4::getMove(Connect4State state) {
...
int MinMaxPossible4HeuristicD4::maxValue(Connect4State state, int depth) {
  if (state.gameOver())
...
int MinMaxPossible4HeuristicD4::minValue(Connect4State state, int depth) {
  if (state.gameOver())
    return 1000*state.winner();
...
int MinMaxPossible4HeuristicD4::patternCount(Connect4State state, std::vector<int> sequence) {
  int count = 0;
  uint64_t pattern;
...
int MinMaxPossible4HeuristicD4::heuristic(Connect4State state) {
  if (state.gameOver())
    return 1000*state.winner();
    
  uint64_t position;
  if (state.getTurn() % 2 == 0) {
    position = (1ULL << 42) - std::get<2>(state.getState());
  }
  else {
    position = (1ULL << 42) - std::get<1>(state.getState());
  }
  ...
  h += __builtin_popcountll(current); 
...
PYBIND11_MODULE(MinMaxPossible4HeuristicD4, m) {
  pybind11::class_<MinMaxPossible4HeuristicD4>(m, "MinMaxPossible4HeuristicD4")
      .def(pybind11::init<>())
      .def("getMove", &MinMaxPossible4HeuristicD4::getMove);
}
