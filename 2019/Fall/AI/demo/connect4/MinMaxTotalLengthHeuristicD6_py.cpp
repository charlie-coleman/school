...
class MinMaxTotalLengthHeuristicD6 {
public:
...
int MinMaxTotalLengthHeuristicD6::getMove(Connect4State state) {
...
int MinMaxTotalLengthHeuristicD6::maxValue(Connect4State state, int depth) {
  if (state.gameOver())
...
int MinMaxTotalLengthHeuristicD6::minValue(Connect4State state, int depth) {
  if (state.gameOver())
    return 1000*state.winner();
...
int MinMaxTotalLengthHeuristicD6::patternCount(Connect4State state, std::vector<int> sequence) {
  int count = 0;
  uint64_t pattern;
...
int MinMaxTotalLengthHeuristicD6::heuristic(Connect4State state) {
  if (state.gameOver())
    return 1000*state.winner();
...
PYBIND11_MODULE(MinMaxTotalLengthHeuristicD6, m) {
  pybind11::class_<MinMaxTotalLengthHeuristicD6>(m, "MinMaxTotalLengthHeuristicD6")
      .def(pybind11::init<>())
      .def("getMove", &MinMaxTotalLengthHeuristicD6::getMove);
}
