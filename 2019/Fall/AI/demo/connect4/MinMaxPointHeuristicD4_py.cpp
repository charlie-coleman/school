...
int MinMaxPointHeuristicD4::getMove(Connect4State state) {
...
int MinMaxPointHeuristicD4::maxValue(Connect4State state, int depth) {
  if (state.gameOver())
...
int MinMaxPointHeuristicD4::minValue(Connect4State state, int depth) {
  if (state.gameOver())
    return 1000*state.winner();
...
int MinMaxPointHeuristicD4::patternCount(Connect4State state, std::vector<int> sequence) {
  int count = 0;
  uint64_t pattern;
...
int MinMaxPointHeuristicD4::heuristic(Connect4State state) {
  if (state.gameOver())
    return 1000*state.winner();
...
PYBIND11_MODULE(MinMaxPointHeuristicD4, m) {
  pybind11::class_<MinMaxPointHeuristicD4>(m, "MinMaxPointHeuristicD4")
      .def(pybind11::init<>())
      .def("getMove", &MinMaxPointHeuristicD4::getMove);
}
