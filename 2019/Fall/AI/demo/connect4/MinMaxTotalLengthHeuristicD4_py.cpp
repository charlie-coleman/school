...
class MinMaxTotalLengthHeuristicD4 {
public:
...
int MinMaxTotalLengthHeuristicD4::getMove(Connect4State state) {
...
int MinMaxTotalLengthHeuristicD4::maxValue(Connect4State state, int depth) {
  if (state.gameOver())
...
int MinMaxTotalLengthHeuristicD4::minValue(Connect4State state, int depth) {
  if (state.gameOver())
    return 1000*state.winner();
...
int MinMaxTotalLengthHeuristicD4::patternCount(Connect4State state, std::vector<int> sequence) {
  int count = 0;
  uint64_t pattern;
...
int MinMaxTotalLengthHeuristicD4::heuristic(Connect4State state) {
  if (state.gameOver())
    return 1000*state.winner();
    
  int h = 0;
  h += patternCount(state, {1,0});
  h += patternCount(state, {1,1,0});
  h += patternCount(state, {1,1,1,0});
  h += patternCount(state, {0,1});
...
PYBIND11_MODULE(MinMaxTotalLengthHeuristicD4, m) {
  pybind11::class_<MinMaxTotalLengthHeuristicD4>(m, "MinMaxTotalLengthHeuristicD4")
      .def(pybind11::init<>())
      .def("getMove", &MinMaxTotalLengthHeuristicD4::getMove);
}
